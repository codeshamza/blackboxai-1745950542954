...
async function updateDashboard() {
  ...
  const averagePingElem = document.getElementById("average-ping");
  ...
  // Clear table body before first load
  if (!window.hasLoadedOnce) {
    const tbody = document.getElementById("signal-tbody");
    tbody.innerHTML = "";
  }
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    try {
      ...
      if (!row) {
        ...
        document.getElementById(`action-${pair}`).textContent = signals.action;
        document.getElementById(`action-${pair}`).className = `px-4 py-2 font-bold ${signals.action === "BUY" ? "text-green-700" : signals.action === "SELL" ? "text-red-700" : "text-yellow-500"}`;

        document.getElementById(`jbSafe-${pair}`).textContent = signals.jbSafe ? "✔️" : "❌";
        document.getElementById(`jbSafe-${pair}`).className = `px-4 py-2 ${signals.jbSafe ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}`;

        document.getElementById(`volComp-${pair}`).textContent = signals.volatilityCompression ? "✔️" : "❌";
        document.getElementById(`volComp-${pair}`).className = `px-4 py-2 ${signals.volatilityCompression ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}`;

        document.getElementById(`trend-${pair}`).textContent = signals.trend;
        document.getElementById(`trend-${pair}`).className = `px-4 py-2 ${signals.trend === 'UP' ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}`;

        // Color RSI value based on thresholds
        let rsiClass = 'px-4 py-2 ';
        const rsiValNum = parseFloat(signals.rsi);
        if (!isNaN(rsiValNum)) {
          if (rsiValNum < 30) {
            rsiClass += 'text-red-600 font-semibold';
        ...
        const rsiElem = document.getElementById(`rsi-${pair}`);
        rsiElem.textContent = signals.rsi;
        rsiElem.className = rsiClass;

        document.getElementById(`macd-${pair}`).textContent = signals.macdStatus;
        document.getElementById(`macd-${pair}`).className = `px-4 py-2 ${signals.macdStatus === 'Bullish' ? 'text-green-600 font-semibold' : signals.macdStatus === 'Bearish' ? 'text-red-600 font-semibold' : 'text-yellow-500 font-semibold'}`;

        document.getElementById(`vwap-${pair}`).textContent = signals.vwapStatus;
        document.getElementById(`vwap-${pair}`).className = `px-4 py-2 ${signals.vwapStatus === 'Above' ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}`;

        document.getElementById(`volChange-${pair}`).textContent = signals.volatilityChange;
        document.getElementById(`volChange-${pair}`).className = `px-4 py-2 ${signals.volatilityChange === 'Up' ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}`;

        document.getElementById(`jbChange-${pair}`).textContent = signals.jbChangeStatus;
        document.getElementById(`jbChange-${pair}`).className = `px-4 py-2 ${signals.jbChangeStatus === 'Up' ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}`;

        // Remove position size cell entirely by removing the element if exists
        const posSizeElem = document.getElementById(`posSize-${pair}`);
        if (posSizeElem && posSizeElem.parentNode) {
          posSizeElem.parentNode.removeChild(posSizeElem);
        }

        // Update price with bold and dark font
        const priceElem = document.getElementById(`price-${pair}`);
        priceElem.textContent = currentPrice !== null ? currentPrice.toFixed(4) : 'N/A';
        priceElem.className = 'px-4 py-2 font-mono text-right font-extrabold text-gray-900';
      }
    } catch (error) {
  ...
  // Delay showing table and hiding loading screen to ensure CSS is applied
  setTimeout(() => {
    loading.style.display = "none";
    document.getElementById("signal-table").classList.remove("hidden");

    // Hide loading screen after table is loaded
  ...
  // Update last updated time with relative seconds ago
  const now = new Date();
  if (!window.lastUpdateTime) {
    window.lastUpdateTime = now;
  }
  const secondsAgo = Math.floor((now - window.lastUpdateTime) / 1000);
  lastUpdatedElem.textContent = `Last Updated: ${secondsAgo <= 25 ? secondsAgo : 25} seconds ago`;
  // Do not update lastUpdateTime here to keep the original update time for relative counting

  // Calculate average ping
  if (pings.length > 0) {
    const avgPing = pings.reduce((a, b) => a + b, 0) / pings.length;
    averagePingElem.textContent = `Average Ping: ${avgPing.toFixed(0)} ms`;
    if (avgPing < 200) {
      averagePingElem.className = "text-green-600 font-semibold";
    } else if (avgPing < 500) {
      averagePingElem.className = "text-yellow-600 font-semibold";
    } else {
      averagePingElem.className = "text-red-600 font-semibold";
    }
  } else {
    averagePingElem.textContent = "";
  }
};
...
window.onload = () => {
  ...
  // Update last updated seconds ago every second
  setInterval(() => {
    const lastUpdatedElem = document.getElementById("last-updated");
    if (window.lastUpdateTime && lastUpdatedElem) {
      const now = new Date();
      const secondsAgo = Math.floor((now - window.lastUpdateTime) / 1000);
      lastUpdatedElem.textContent = `Last Updated: ${secondsAgo} seconds ago`;
    }
  }, 1000);
};
